...
### Latest
> **A work in progress:**
- Default TimeFrom mode is Manual. If Video Filename has DateTime string on end then that is interpreted as the Gun (race start) DateTime.
...
- Matching change coming in Video Capture Phone app.
  - _Could also check if audio in video and set that mode or even check video frames for flash mnode._
